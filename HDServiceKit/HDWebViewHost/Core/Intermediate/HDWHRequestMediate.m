...
+ (int)_above_iOS11_interMediateFile:(NSString *)fileName inDirectory:(NSURL *)directory output:(NSString **)output {
    ...
    for (HTMLNode *inputNode in inputNodes) {
        ...
        if ([HDWHUtil isNetworkUrl:linkedCss]) {
            continue;
        }

        NSString *originalStyle = inputNode.rawContents;
        HDWHLog(@"style src = %@, rawContents = %@", linkedCss, originalStyle);
        if (originalStyle.length > 0) {
            NSString *path = [[[directory URLByAppendingPathComponent:linkedCss] absoluteString] stringByReplacingOccurrencesOfString:kFilePrefix withString:HDWebViewHostURLStyleServer];
            ...
            ext = -2;
            HDWHLog(@"Replace linked css error, originalStyle = %@", originalStyle);
        }
    }
    ...
    for (HTMLNode *spanNode in spanNodes) {
        ...
        if ([HDWHUtil isNetworkUrl:linkedScript]) {
            continue;
        }
        if (linkedScript.length > 0) {
            NSString *originalScript = spanNode.rawContents;
            HDWHLog(@"script src = %@, rawContents = %@", linkedScript, originalScript);
            if (originalScript.length > 0) {
                NSString *path = [[[directory URLByAppendingPathComponent:linkedScript] absoluteString] stringByReplacingOccurrencesOfString:kFilePrefix withString:HDWebViewHostURLScriptServer];
                ...
                ext = -3;
                HDWHLog(@"Replace linked script originalScript = %@", originalScript);
            }
        }
    }
    ...
    for (HTMLNode *spanNode in imgNode) {
        ...
        if ([HDWHUtil isNetworkUrl:imgSrc]) {
            continue;
        }
        if (imgSrc.length > 0) {
            NSString *originalSrc = spanNode.rawContents;
            HDWHLog(@"image src = %@, rawContents = %@", imgSrc, originalSrc);
            if (imgSrc.length > 0) {
                NSError *error = nil;
                ...
                NSString *path = [[[directory URLByAppendingPathComponent:imgSrc] absoluteString] stringByReplacingOccurrencesOfString:kFilePrefix withString:HDWebViewHostURLImageServer];
                ...
                ext = -4;
                HDWHLog(@"Replace linked script originalScript = %@", imgSrc);
            }
        }
    }
...
+ (int)_below_iOS11_interMediateFile:(NSString *)fileName inDirectory:(NSURL *)directory output:(NSString **)output {
    ...
    for (HTMLNode *inputNode in inputNodes) {
        ...
        if (err == nil && originalStyle.length > 0 && content.length > 0) {
            ...
            ext = -2;
            HDWHLog(@"Replace linked css error = %@ , originalStyle = %@, content = %@", err, originalStyle, content);
        }
    }
    ...
    for (HTMLNode *spanNode in spanNodes) {
        ...
        if (linkedScript.length > 0) {
            ...
            if (err == nil && originalScript.length > 0 && content.length > 0) {
                ...
                ext = -3;
                HDWHLog(@"Replace linked script error = %@, originalScript = %@, content = %@", err, originalScript, content);
            }
        }
    }
